...
export class AuthService {
  ...
  async login(email: string, password: string, ip?: string, userAgent?: string) {
    ...
    await this.refresh.persist(user.id, refresh.token, refresh.jti, refresh.expiresAt, ip, userAgent);
    await this.user.updateUser({ where: { id: user.id }, data: { lastLoginAt: new Date() } });
    this.logger.log(`Login success for ${user.email} (${user.id})`);
    return { user, access, refresh };
  }

  async refreshTokens(userId: string, jti: string, rawRefreshToken: string, ip?: string, userAgent?: string) {
    const validation = await this.refresh.validateAndGet(jti, rawRefreshToken);
    if (validation.status !== 'valid' || !validation.token || validation.token.userId !== userId) {
      if (validation.status === 'revoked' || validation.status === 'mismatch') {
        await this.refresh.revokeAllUserTokens(userId, 'token-reuse', ip);
        this.logger.warn(`Detected refresh token reuse for user ${userId} from ${ip ?? 'unknown ip'}`);
      }
    ...
    await this.refresh.rotate(jti, { token: next.token, jti: next.jti, expiresAt: next.expiresAt }, ip, userAgent);
