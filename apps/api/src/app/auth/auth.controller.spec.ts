...
describe('AuthController', () => {
  ...
  beforeEach(() => {
    process.env = { ...originalEnv, JWT_REFRESH_TTL: '10s', API_PREFIX: 'api', NODE_ENV: 'production' };
    auth = {
      login: jest.fn(),
  ...
  it('rejects login without email or password', async () => {
    ...
    await expect(controller.login({ email: '', password: '' } as any, req, res)).rejects.toBeInstanceOf(UnauthorizedException);
  });
  ...
  it('logs in and sets refresh cookie', async () => {
    ...
    auth.login.mockResolvedValue({
      user: { id: 'u1', email: 'u@test.com', role: 'ADMIN', avatar: null },
      access: { token: 'at', expiresAt },
      refresh: { token: 'rt' },
    });

    const result = await controller.login({ email: 'u@test.com', password: 'pass' }, req, res);

    expect(auth.login).toHaveBeenCalledWith('u@test.com', 'pass', '1.1.1.1', 'UA');
    expect(res.cookie).toHaveBeenCalledWith(
      'refresh_token',
      'rt',
      expect.objectContaining({ httpOnly: true, secure: true, sameSite: 'none', path: '/api/auth/refresh', maxAge: 10_000 }),
    );
  ...
  it('rejects refresh when token missing', async () => {
    const res = { cookie: jest.fn(), clearCookie: jest.fn() } as any;
    const req = { refresh: { token: undefined }, headers: {}, ip: '1.1.1.1' } as any;
  ...
  it('refreshes tokens and updates cookie', async () => {
    ...
    const res = { cookie: jest.fn(), clearCookie: jest.fn() } as any;
    const req = { refresh: { token: 'rt', jti: 'j1', userId: 'u1' }, headers: { 'user-agent': 'UA2' }, ip: '2.2.2.2' } as any;
    const expiresAt = new Date('2024-02-02T00:00:00Z');
    auth.refreshTokens.mockResolvedValue({
      user: { id: 'u1', email: 'u@test.com', role: 'USER', avatar: 'x' },
    ...
    expect(auth.refreshTokens).toHaveBeenCalledWith('u1', 'j1', 'rt', '2.2.2.2', 'UA2');
    expect(res.cookie).toHaveBeenCalledWith(
      'refresh_token',
      'new-rt',
      expect.objectContaining({ secure: false, sameSite: 'strict', path: '/api/auth/refresh' }),
    );
    expect(result).toEqual({
      accessToken: 'new-at',
      accessTokenExpiresAt: expiresAt,
  ...
  it('returns current user from me route', () => {
    const user = { id: 'u1', email: 'u@test.com', roles: 'ADMIN', avatar: 'a' };
    expect(controller.me(user as any)).toEqual({ id: 'u1', email: 'u@test.com', role: 'ADMIN', avatar: 'a' });
  });
});
