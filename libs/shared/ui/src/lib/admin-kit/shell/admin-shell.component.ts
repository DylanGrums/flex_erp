...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, RouterModule, ButtonModule, AvatarModule, TranslocoModule],
    template: `
    <div class="flex h-screen w-full bg-ui-bg-base text-ui-fg-base overflow-hidden">
      ...
      <aside class="w-64 border-r border-ui-border-base flex flex-col bg-ui-bg-base flex-shrink-0">
        ...
        <div class="h-14 flex items-center px-4 border-b border-ui-border-base">
          ...
          <span class="font-semibold text-sm">{{ 'admin.shell.brand' | transloco }}</span>
        </div>
        ...
        <div class="p-3 border-t border-ui-border-base">
            ...
            <div class="mt-3 pt-3 border-t border-ui-border-base flex items-center gap-3 px-3">
                 ...
                 <div class="flex flex-col">
                     <span class="text-xs font-medium text-ui-fg-base">{{ 'admin.shell.user' | transloco }}</span>
                     <span class="text-2xs text-ui-fg-muted">{{ 'admin.shell.userEmail' | transloco }}</span>
                 </div>
            </div>
        </div>
      </aside>
      ...
      <main class="flex-1 flex flex-col min-w-0 bg-ui-bg-subtle/30">
        ...
        <header class="h-14 border-b border-ui-border-base bg-ui-bg-base flex items-center justify-between px-6 flex-shrink-0 sticky top-0 z-10">
          <div class="flex items-center gap-4">
            <h1 class="text-sm font-medium text-ui-fg-base">{{ pageTitle | transloco }}</h1>
          </div>
...
export class FeAdminShellComponent {
    ...
    navItems = [
        { label: 'admin.nav.search', icon: 'pi pi-search', link: '/search' },
        { label: 'admin.nav.orders', icon: 'pi pi-shopping-cart', link: '/orders' },
        { label: 'admin.nav.products', icon: 'pi pi-tags', link: '/products' },
        { label: 'admin.nav.collections', icon: 'pi pi-th-large', link: '/collections' },
        { label: 'admin.nav.categories', icon: 'pi pi-list', link: '/categories' },
        { label: 'admin.nav.inventory', icon: 'pi pi-box', link: '/inventory' },
        { label: 'admin.nav.customers', icon: 'pi pi-users', link: '/customers' },
        { label: 'admin.nav.promotions', icon: 'pi pi-percentage', link: '/promotions' },
        { label: 'admin.nav.priceLists', icon: 'pi pi-dollar', link: '/price-lists' },
    ];
}
