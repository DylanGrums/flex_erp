...
import { ProductOption, ProductVariantDetail } from '@flex-erp/store/util';
...
@Component({
  selector: 'fe-product-variant-edit-dialog',
  standalone: true,
    ...
    >
      <div class="flex h-full flex-col">
        <header class="flex items-center justify-between border-b border-ui-border-base px-6 py-4">
          <div>
            <div class="text-base font-semibold text-ui-fg-base">Edit variant</div>
            <div class="text-xs text-ui-fg-muted">Update SKU, price, and option values.</div>
          </div>
          <button
            type="button"
...
export class ProductVariantEditDialogComponent {
  ...
  private readonly variant = signal<ProductVariantDetail | null>(null);
  ...
  constructor() {
    effect(() => {
      ...
      if (!detail || this.hydrated() || !this.variantId) {
        return;
      }

      const selected = detail.variants?.find((item) => item.id === this.variantId) ?? null;
      if (!selected) {
        this.options.set([]);
        return;
      }
      ...
      this.variant.set(selected);
      this.form.patchValue({
        sku: selected.sku ?? null,
        price: selected.price ?? (selected.priceAmount ? selected.priceAmount / 100 : 0),
      });

      const controls = ordered.map((option) => {
        const value = selected.optionValues?.find((item) => item.optionId === option.id)?.value ?? 'Default';
  ...
  async onSubmit() {
    if (this.form.invalid || !this.variantId) {
      return;
    }
    ...
    const index = detail.variants?.findIndex((variant) => variant.id === this.variantId) ?? -1;
    if (index < 0) {
      return;
    }
